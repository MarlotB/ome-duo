...
public class RepeatingBackground : MonoBehaviour 
{
    ...
    private float originalGroundHorizontalLength;
    ...
    private void Awake ()
    {
		...
		groundHorizontalLength = groundCollider.size.x + transform.position.x;
        originalGroundHorizontalLength = groundCollider.size.x;
    }
    ...
    private void Update()
    {
        ...
        if (mainCamera.transform.position.x > groundHorizontalLength)
        {
    ...
    private void RepositionBackground()
    {
		...
		Vector2 groundOffSet = new Vector2(originalGroundHorizontalLength * 2f, 0);

        //Move this object from it's position offscreen, behind the player, to the new position off-camera in front of the player.
        transform.position = (Vector2) transform.position + groundOffSet;
		groundHorizontalLength = groundHorizontalLength + transform.position.x;
    }
}